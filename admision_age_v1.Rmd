...
library(tidyverse);library(data.table);library(lubridate); library(zoo); library(patchwork);library(survival)
...
base_final <- base_trabajo %>% 
  left_join(base_cdmx_v2, by = "fecinisi") %>% 
  mutate(hospitalizado = case_when(is.na(lugarmuerte)~T,
                                   lugarmuerte=="Hospital"~T,
                                   lugarmuerte=="Domicilio"~F))
```
...
table(MUERTES=base_final$muerte,
      LUGAR=base_final$hospitalizado)
range(base_final$daily_cases)
```
